...
public class ServerBackground {
	...
	private ServerGUI gui; // gui의 주소값이 들어와야함

	// 사용자들의 정보를 지정하는 맵 객체 선언 nickname = key //닉네임을 넣고 아웃풋만 관리하겠다는 의미
	// client가 메세지를 넣으면 그걸 모든 client들에게 보내기 위해 이렇게 묶음
	...
	// 서버 생성 - 서버 정보 설정
		...
		public void sendMessage(String receivedMsg,String nickname) { //nickname : 방금 수신 메세지를 보낸 client
			for(String key : clientMap.keySet()) {
				try {
//					DataOutputStream out = clientMap.get(key);
//					out.writeUTF(receivedMsg);
					//if(!key.equals(nickname)) { // nickname이 다른 client한테만 보내줘 
						clientMap.get(key).writeUTF(receivedMsg);
					//}
					System.out.printf("수신자 :%s------%s------\n",nickname,key);
				} catch (IOException e) {
					// e.printStackTrace();
				}
			}
		...
		class Receiver extends Thread {
			...
			public Receiver(Socket socket) {
				try {
					...
					nickname = in.readUTF();
					// map에 접속한 사용자의 정보 추가
					//server입장에서 output만 관리하겠다는 것
					//server에 들어온 값을 모든 client에 전송
